...
public class PathGenerator_original : MonoBehaviour
{
    ...
    private void GenerateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                grid[x, y] = new PathNode(x, y, new List<Vector2Int>(possibleDirections));
            }
        }
    }
    ...
    private void CloneGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                PathNode clonedNode = new PathNode(originalNode.posX, originalNode.posY, new List<Vector2Int>(originalNode.possibleDirections));
    ...
    private IEnumerator GeneratePath()
    {
        ...
        //Debug.Break();
        while (currentPath.Count != pathLength || currentPath[currentPath.Count - 1].Position != new Vector2Int(endPosX, endPosY))
        {
            ...
            {
                Vector2Int direction = currentNode.possibleDirections[UnityEngine.Random.Range(0, currentNode.possibleDirections.Count)];
